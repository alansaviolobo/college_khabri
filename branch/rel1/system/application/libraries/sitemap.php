...
class Sitemap {
    ...
    private $template_file = 'sitemap.html'; // optional sitemap template file
    ...
    function ignore($controller, $pages)
    {
        ...
        if(is_array($pages)) {
            array_walk_recursive($pages, array('Sitemap', 'stl_callback'));
        }
        else {
            $pages = strtolower($pages);
        }

        if(array_key_exists($controller, $this->ignore) && is_array($this->ignore[$controller])) $pages = array_merge($this->ignore[$controller], (array) $pages);
    ...
    function get_links($class = null)
    {
        ...
        foreach($controller->getMethods() as $method)
        {
            ...
            if(in_array(strtolower($method->name), (array) $this->ignore)) continue;//index removed [strtolower($class)]
    ...
    function generate()
    {
        ...
        foreach(get_filenames($controllers_path, true) as $controller) {
            ...
            if(array_key_exists(strtolower($class), $this->ignore) && $this->ignore[strtolower($class)] == '*') continue;    // skip controllers marked as 'ignore'
